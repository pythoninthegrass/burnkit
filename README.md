...
Reads a [Deepseek Harness](https://www.deepseek.com/harness/en/) headless-session JSONL log (zstd-compressed, streamed once) so a finished run can be audited rather than trusted. Install the extra for the `raw`/`search` subcommands, which evaluate user-supplied jq filters:
